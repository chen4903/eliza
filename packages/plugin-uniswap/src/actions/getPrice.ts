...
export const getPriceAction: Action = {
    ...
    handler: async (
        ...
        // TODO: Support users to input information to obtain the price of specified tokens
        // const content =
        ...
        if (callback) {
            await callback({
                text: `token-0 price in pair: ${route.midPrice.toSignificant(6)}, token-1 price in pair: ${route.midPrice.invert().toSignificant(6)}`,
                action: "GET_PRICE"
            });
        }
    ...
    examples: [
        [
            {
                user: "{{user1}}",
                content: { text: "please show me the token price" },
            },
            {
                user: "{{system}}",
